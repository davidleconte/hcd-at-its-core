...
| `remediate-worktree` / `-clean` / `-record` | Manual remediation-worktree plumbing |
...
| **HCD-I2** | D6 | Secure cluster forms (6× UN) | live |
...
count + content sha256 (generated artefacts excluded so the hash is stable), tool versions
(python/shellcheck/pytest/cqlsh), the Oracle summary, and the seven invariant statuses. Cross-checked
...
block). It is wired two ways: the git **pre-push hook** (`bin/pre-merge-hook.sh`, installable via
`make audit-install-hook`) and the **CI** `audit-arena` job. The hook is read-only on *tracked* files
but, on success, refreshes the courtroom snapshot (§4.10).
...
|---|---|---|---|
| **R1** | manual MECE seed (Claude) | 14 (4 BLOCKER incl. live-breaking CQL, secure cluster) | all FIXED |
