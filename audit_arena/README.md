...
> Generated arena outputs (`courtroom.html`, `REPO_MAP.md`, `oracle_r*`/`invariants_r*`/`manifest_r*`/
> `lineage_r*`/`panel_r*`/`vendor_panel_r*`/`remediation_r*`/`*_r*__<vendor>`.json, `reconciliation.json`,
> `state/forge/`, `patches/`) are git-ignored and regenerated by `make audit`; only the seed
> `findings/verdicts/grades_r1`, the contract (`contract/contract.v1.json`), the forge contracts
> (`forge/*.contract.json`), and the engine code are tracked.
...
```bash
make audit               # contract -> repomap -> oracle -> invariants -> lineage -> manifest ->
                         #   panel-aggregate -> reconcile -> render -> gate  (-> courtroom.html)
...
Beyond the v1 tribunal, the engine now ships a **honesty reconciler** (`reconcile` — a numeric judge
score is advisory and read by no gate; a judge-green over a DEFERRED invariant is flagged, not green;
see [DESIGN_honesty_guardrails.md](DESIGN_honesty_guardrails.md)), a versioned **contract spine**
(`contract` — the Definition-of-Done as data), per-finding **lineage** (`lineage` — one Oracle-dominant
provenance object, + a manifest `audit_root_sha256`), an advisory **scored panel** with the Oracle
ceiling re-derived in code (`panel-aggregate`), a routine **multi-vendor panel** (`vendor-panel`), a
generative **forge** battle (`forge-contract`/`-sign`/`-verify`/`-record`/`-converge` — design an
artefact against a human-signed contract), and an interactive **pupitre** console in `courtroom.html`
(with `replay <ids>` as the safe execution path).
