...
Usage:
    driver-demo token-aware   [--contact-points IPS] [--keyspace KS]
...
def cmd_token_aware(args):
    ...
    for i in range(30):
        ...
        coord = str(result.response_future.coordinator_host)
...
def cmd_dc_failover(args):
    ...
    while (time.monotonic() - start_time) < duration:
        ...
        try:
            ...
            print(f"[WRITE] row={row_id:3d}  coordinator=???           status=FAIL ({ename})")
...
class AggressiveRetryPolicy(RetryPolicy):
    """Custom retry policy: retry on the next host up to 3 times."""
...
def cmd_retry_policies(args):
    ...
    for i in range(20):
        try:
            ...
            errors += 1
            ename = type(e).__name__
            print(f"[WRITE] row={i:2d}  coordinator=???           status=FAIL ({ename})")
